...
Most extensions are just a new credential the existing credential rail already serves — with
**no new code path** (007). Define it with `defineCredential` + `dcql` + an effect builder
...
**How "no new code path" holds (007).** `requirements()` registers each policy credential by
id (register-on-resolve, no developer registration), and `mount()` injects that registry into
the ceremony context. The mounted credential-gate rail then serves ANY registered credential
from its own `request`/`verify` — not just the `age`/`membership` built-ins — and `completeOrder`
sweeps every applicable `gate()` credential, re-derives applicability from the re-priced order
(invariant 2), and refuses one that isn't proven for that order (invariants 1/4). A `gate()` is
the hard-block effect: it is enforced whenever it applies, independent of `required(...)` /
`optional(...)` (which govern the non-blocking `discount()`/`authorize()` effects). The runnable
worked pack is [`examples/professional-license.mjs`](../../examples/professional-license.mjs);
`verify` runs server-side and never crosses the wire (Principle VI), and every custom surface
stays `trust_level: "presence-only-demo"` until issuer-verified trust lands (#14). *Multi-instance
note:* register-on-resolve means each instance must resolve the policy once (e.g. at startup)
before it enforces a custom gate — the reference single-server/demo deployment always does;
credential definitions are code on every instance, only state is shared.
