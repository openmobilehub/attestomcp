...
export class AttestoMCP {
  ...
  constructor(opts: AttestoMCPOptions = {}) {
    ...
    if (!origin) {
      ...
      console.warn(
        `[attestomcp] walletOrigin "${origin}" is not an absolute http(s) origin; using ${DEFAULT_WALLET_ORIGIN}. ` +
    ...
    if (process.env.NODE_ENV === "production" && /^https?:\/\/(localhost|127\.0\.0\.1)/.test(origin)) {
      console.warn(
        `[attestomcp] walletOrigin is ${origin} in production — buyers can't open localhost approve links. ` +
