...
describe("CT8 — a dc-payment verify completes via the shared completeOrder seam", () => {
  ...
  it("is idempotent — a replayed verify echoes the record and writes/settles nothing twice", async () => {
    ...
    const first = await request(h.app).post("/attestomcp/dc-payment/verify").send(body);
    expect(first.body.completed).toBe(true);
    expect(h.writes()).toBe(1);

    const replay = await request(h.app).post("/attestomcp/dc-payment/verify").send(body);
...
describe("CT8 — a tampered amount or order id is refused (amount re-derived from the catalog)", () => {
  ...
  it("re-prices a hand-edited (tampered) stored total from the catalog, never the token", async () => {
    ...
    const ok = await request(h.app).post("/attestomcp/dc-payment/verify").send({ order: "ORD-T2", amount: 199, claims: DEMO_CLAIMS });
    expect(ok.body.completed).toBe(true);
    const bad = harness();
    bad.seed("ORD-T2", [{ id: "aurora-headphones", quantity: 1 }], 1);
    const refused = await request(bad.app).post("/attestomcp/dc-payment/verify").send({ order: "ORD-T2", amount: 1, claims: DEMO_CLAIMS });
  ...
  it("the verify handler refuses an unknown order id (400) — the amount has no catalog source", async () => {
    const h = harness();
    const res = await request(h.app).post("/attestomcp/dc-payment/verify").send({ order: "ORD-UNKNOWN", amount: 199, claims: DEMO_CLAIMS });
  ...
  it("the page route 404s an unknown order id", async () => {
    const h = harness();
    const res = await request(h.app).get("/attestomcp/dc-payment").query({ order: "ORD-UNKNOWN" });
...
describe("CT8 — the shared completion seam refuses an unverified age-restricted order", () => {
  it("an age-restricted order with no proven age claim is refused even with a valid payment", async () => {
    ...
    const res = await request(h.app).post("/attestomcp/dc-payment/verify").send({ order: "ORD-AGE", amount: order!.total, claims: DEMO_CLAIMS });
  ...
  it("the same order completes once a positive per-order age claim exists", async () => {
    ...
    const res = await request(h.app).post("/attestomcp/dc-payment/verify").send({ order: "ORD-AGE2", amount: order!.total, claims: DEMO_CLAIMS });
    expect(res.body.completed).toBe(true);
...
describe("CT8 — a membership-discounted order authorizes the discounted total (no path divergence)", () => {
  it("the bound amount equals the re-derived discounted total and completes", async () => {
    ...
    const res = await request(h.app).post("/attestomcp/dc-payment/verify").send({ order: "ORD-M", amount: order!.total, claims: DEMO_CLAIMS });
    expect(res.body.completed).toBe(true);
...
describe("CT11 — page / request descriptor / receipt all state presence-only-demo", () => {
  ...
  it("the verify receipt (mandate) carries trust_level presence-only-demo", async () => {
    ...
    const res = await request(h.app).post("/attestomcp/dc-payment/verify").send({ order: "ORD-H", amount: order!.total, claims: DEMO_CLAIMS });
  ...
  it("the encrypted wallet-presentation path is REAL — it requires a readerContextToken (400, not 501) and records nothing without one", async () => {
    ...
    const res = await request(h.app).post("/attestomcp/dc-payment/verify").send({ order: "ORD-P", result: { protocol: "openid4vp-v1-signed", data: { response: "<jwe>" } } });
    expect(res.status).toBe(400);
    expect(res.body.completed).toBeFalsy();
    expect(h.records.size).toBe(0);
  });
