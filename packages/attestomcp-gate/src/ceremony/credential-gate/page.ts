...
// /attestomcp/credential/request so no await sits between the click and get() — iOS WebKit
// drops the transient user activation across an await), then POSTs the wallet result to
// /attestomcp/credential/verify in the shape the route's real path reads
