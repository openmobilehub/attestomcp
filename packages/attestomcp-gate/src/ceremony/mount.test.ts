...
describe("mountCeremony — fail fast on missing seams (CT2)", () => {
  it("succeeds with a full, valid seam set and exposes the store on app.locals", () => {
    ...
    expect((app.locals.attestomcp as { store?: unknown }).store).toBe(seams.verificationStore);
  });
  ...
  it("reads seams from app.locals.attestomcp when not passed as options", () => {
    const app = fakeApp();
    app.locals.attestomcp = validSeams();
