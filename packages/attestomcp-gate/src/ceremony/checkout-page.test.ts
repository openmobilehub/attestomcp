...
// the demo's `/credential-gate/*` links and the storefront's `/attestomcp/*` links both
...
const manifest: VerificationManifestEntry[] = [
  { credential: "age", required: true, effect: "gate", enforcedAt: "checkout", trust_level: "presence-only-demo", label: "Age 21+", minAge: 21, approveUrl: "/attestomcp/credential?order=ORD-T030&cred=age" },
  { credential: "membership", required: false, effect: "discount", enforcedAt: "checkout", trust_level: "presence-only-demo", label: "10% member discount", discountPct: 10, approveUrl: "/attestomcp/credential?order=ORD-T030&cred=membership" },
  { credential: "payment", required: true, effect: "authorize", enforcedAt: "checkout", trust_level: "presence-only-demo", label: "Pay (USD)", approveUrl: "/attestomcp/dc-payment?order=ORD-T030" },
];
...
describe("renderRequirements — numbered gates + live status", () => {
  ...
  it("links each gate to its OWN approveUrl (route-agnostic — no hardcoded routes)", () => {
    ...
    expect(html).toContain("/attestomcp/credential?order=ORD-T030&amp;cred=age");
    expect(html).toContain("/attestomcp/credential?order=ORD-T030&amp;cred=membership");
    ...
    expect(demoHtml).not.toContain("/attestomcp/");
  });
