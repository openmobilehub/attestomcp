...
// /credentagent/credential/request so no await sits between the click and get() — iOS WebKit
// drops the transient user activation across an await), then POSTs the wallet result to
// /credentagent/credential/verify in the shape the route's real path reads
...
export function renderCredentialPage(args: CredentialPageArgs): string {
  ...
  // gate is step 0 (Age) of Age · Membership · Pay; membership is the middle step; a
  // custom gate shows a two-step [<label> · Pay] rail.
  ...
  const rail = isAge
    ...
    : isMembership
      ...
      : progressRail([{ label: customLabel }, { label: "Pay" }], 0);
  void isCustom;
