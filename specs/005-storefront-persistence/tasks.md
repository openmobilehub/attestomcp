...
testable increment. All paths are under `packages/credentagent-storefront/` unless noted.
...
- [X] T001 Add `@upstash/redis` (^1.x) as an **optional peerDependency** + a `devDependency`, with `peerDependenciesMeta: { "@upstash/redis": { optional: true } }`, in `packages/credentagent-storefront/package.json`
- [X] T002 Add the `"./redis"` subpath to `exports` (→ `./dist/redis.d.ts` / `./dist/redis.js`) in `packages/credentagent-storefront/package.json`
...
- [X] T004 Export `CartStore` and `OrderStore` and add the `StorageProvider` interface (`{ cartStore, createdOrderStore, orderStore, verificationStore }`) in `packages/credentagent-storefront/src/state.ts`; re-export `StorageProvider`, `CartStore`, `OrderStore` from the `./server` entry (`packages/credentagent-storefront/src/server.ts`)
- [X] T005 Add `storage?: StorageProvider` to `StorefrontOptions` and change the four store-resolution lines in `createStorefront` to `opts.<slot> ?? opts.storage?.<slot> ?? new Memory<slot>()` (cart, createdOrder, order, verification) in `packages/credentagent-storefront/src/server.ts` (depends on T004)
...
- [X] T006 [US1] Add a `Map`-backed `RedisLike` fake (`get`/`set`/`del`) as a test helper in `packages/credentagent-storefront/src/redis.test.ts`
- [X] T007 [US1] Failing cross-instance round-trip test (V2/CT-8): two `redisStorage` providers over one fake client + same namespace — write via A, read via B — for cart, created-order, completed-order, verification, in `packages/credentagent-storefront/src/redis.test.ts`
- [X] T008 [US1] Failing per-order isolation/bypass test (V4/CT-9, Security Inv #4): prove verification for `ORD-X`, read `ORD-Y` → unverified; add a comment asserting the test must fail if the `:${orderId}` key segment is removed, in `packages/credentagent-storefront/src/redis.test.ts`

### Implementation for User Story 1

- [X] T009 [US1] Create `packages/credentagent-storefront/src/redis.ts`: `RedisLike` + `RedisStorageOptions` types, `keyFor(namespace, kind, id?)` helper (`${ns}:cart`, `${ns}:order:created:${id}`, `${ns}:order:completed:${id}`, `${ns}:verification:${id}`), and `RedisCartStore` implementing `CartStore` (Map↔object at the boundary)
- [X] T010 [US1] Add `RedisOrderStore<T>` (per-order-id `get`/`set`/`del`, missing → `null`) covering both the created-order and completed-order slots, in `packages/credentagent-storefront/src/redis.ts`
- [X] T011 [US1] Add `RedisVerificationStore` — full-record `get`/`set`/`del` per order id (no internal merge; missing → `undefined`), in `packages/credentagent-storefront/src/redis.ts`
- [X] T012 [US1] Implement `redisStorage(options): StorageProvider` — build a `Redis` client from `url`+`token` OR use an injected `client`; default `namespace = "credentagent-storefront"`; return the four adapters — in `packages/credentagent-storefront/src/redis.ts`
- [X] T013 [US1] Run `npm --workspace @openmobilehub/credentagent-storefront run test`; make T007 + T008 pass
...
- [X] T014 [US2] Guard test (V1/CT-1): `createStorefront()` with no options serves in-memory and its `./server` module graph does not import `@upstash/redis`, in `packages/credentagent-storefront/src/server.test.ts`
- [X] T015 [US2] Config-error handling + tests (CT-6/CT-7/V7): `redisStorage({})` (neither `client` nor `url`+`token`) throws a clear error; when `url`+`token` are given but `@upstash/redis` cannot be loaded, throw an actionable error naming the missing optional peer dep (via an injectable loader seam so it's testable while the dep is installed) — in `packages/credentagent-storefront/src/redis.ts` + `src/redis.test.ts`
...
- [X] T016 [US3] Override-precedence test (V5/CT-3): assert an injected `orderStore` spy receives the completed-order write while the provider's order store is untouched; also assert other slots still come from the provider, in `packages/credentagent-storefront/src/server.test.ts`
- [X] T017 [US3] Type-export check: `import type { StorageProvider, CartStore, OrderStore }` from `@openmobilehub/credentagent-storefront/server` compiles (escape-hatch typing), in `packages/credentagent-storefront/src/types.test-d.ts`
...
- [X] T018 [US4] Namespace-isolation test (V3/CT-8): two providers over one fake client with namespaces `shop-a` and `shop-b`; write cart/order/verification through each; assert zero cross-namespace reads, in `packages/credentagent-storefront/src/redis.test.ts`
- [X] T019 [US4] Default-namespace test: omitting `namespace` uses `"credentagent-storefront"` and does not collide with an explicitly-namespaced provider, in `packages/credentagent-storefront/src/redis.test.ts`
...
- [X] T020 [P] Fail-closed test (V6/CT-11): an injected `client` whose `get`/`set` reject makes the store op reject; `createStorefront` does not fall back to in-memory, in `packages/credentagent-storefront/src/redis.test.ts`
- [X] T021 [P] Add a concise "Production persistence" section (one `storage: redisStorage(...)` option) to `packages/credentagent-storefront/README.md`
